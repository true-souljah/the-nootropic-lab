...
import { ingredients, productsAU, regionalIngredientNote, regionalTitleQualifier } from '@nootropic/data';
...
export async function generateMetadata({
  ...
  if (!ing) return {};
  const localCount = productsAU.filter((p) => ing.productsContaining.includes(p.slug)).length;
  const q = regionalTitleQualifier('au', localCount, regionalIngredientNote('au', ingredient));
  return {
    ...
    alternates: buildAlternates({ regionCode: 'au', path: `/ingredients/${ingredient}/` }),
    openGraph: buildOpenGraph({ regionCode: 'au', path: `/ingredients/${ingredient}/`, title: `${ing.name}${q} — Nootropic Guide, Dosing, Effects & FAQs`, description: `${ing.name}${q}: mechanism of action, clinical dose (${ing.clinicalDose}), human effect matrix, how to take, stacking guide, and consumer FAQs — products with international shipping.` }),
...
export default async function IngredientPage({
  ...
  const containingProducts = productsAU.filter((p) =>
  ...
  return (
    <IngredientDetail
      ...
      uiStrings={uiStrings}
      regional={{ ...regionalProps(containingProducts), note: regionalIngredientNote('au', ing.slug) }}
    />
  );
}
