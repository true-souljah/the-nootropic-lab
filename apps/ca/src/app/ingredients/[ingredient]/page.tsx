...
import { ingredients, productsCA, regionalIngredientNote, regionalTitleQualifier } from '@nootropic/data';
...
export async function generateMetadata({
  ...
  if (!ing) return {};
  const localCount = productsCA.filter((p) => ing.productsContaining.includes(p.slug)).length;
  const q = regionalTitleQualifier('ca', localCount, regionalIngredientNote('ca', ingredient));
  return {
    ...
    openGraph: buildOpenGraph({ regionCode: 'ca', path: `/ingredients/${ingredient}/`, title: `${ing.name}${q} — Nootropic Guide, Dosing, Effects & FAQs`, description: `${ing.name}${q}: mechanism of action, clinical dose (${ing.clinicalDose}), human effect matrix, how to take, stacking guide, and consumer FAQs — products with international shipping.` }),
...
export default async function IngredientPage({
  ...
  return (
    <IngredientDetail
      ...
      uiStrings={uiStrings}
      regional={{ ...regionalProps(containingProducts), note: regionalIngredientNote('ca', ing.slug) }}
    />
  );
}
