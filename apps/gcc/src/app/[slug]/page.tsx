...
export function generateStaticParams() {
  return productsGCC.map(p => ({ slug: p.slug }));
}
...
export async function generateMetadata({
  ...
  const product = productsGCC.find(p => p.slug === slug);
  if (!product) return {};
  return {
    title: `${product.name} Review 2026 — GCC Buyer's Guide`,
    description: `Independent GCC review of ${product.name}. Score: ${product.score}/10. Clinical dosing audit and full affiliate disclosure.`,
  };
}
...
export default async function ProductReviewPage({
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Best Nootropics', item: 'https://gcc.thenootropiclab.com/best-nootropics' },
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            {product.editorChoice && (
              <span className="editor-badge">Editor&apos;s Choice — GCC 2026</span>
            )}
