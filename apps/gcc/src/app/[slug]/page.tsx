...
export async function generateMetadata({
  ...
  if (!product) return {};
  const title = `${product.name} Review ${CURRENT_YEAR} — GCC Buyer's Guide`;
  const description = `Independent GCC review of ${product.name}. Score: ${product.score}/10. Clinical dosing audit and full affiliate disclosure.`;
  return {
    title,
    description,
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            {product.editorChoice && (
              <span className="editor-badge">Editor&apos;s Choice — GCC {CURRENT_YEAR}</span>
            )}
