...
export function generateStaticParams() {
  return productsJP.map(p => ({ slug: p.slug }));
}
...
export async function generateMetadata({
  ...
  const product = productsJP.find(p => p.slug === slug);
  if (!product) return {};
  return {
    title: `${product.name} Review 2026 — Japan Buyer's Guide`,
    description: `Independent Japan review of ${product.name}. Score: ${product.score}/10. Clinical dosing audit and full affiliate disclosure.`,
  };
}
...
export default async function ProductReviewPage({
  ...
  const breadcrumbSchema = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://jp.thenootropiclab.com' },
      { '@type': 'ListItem', position: 2, name: 'Best Nootropics', item: 'https://jp.thenootropiclab.com/best-nootropics' },
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            {product.editorChoice && (
              <span className="editor-badge">Editor&apos;s Choice — Japan 2026</span>
            )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 my-8">
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="font-bold text-gray-900">
              {product.priceMonthlyJPY ? `¥${product.priceMonthlyJPY.toLocaleString()}` : `$${product.priceMonthlyUSD} USD`}
            </div>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
