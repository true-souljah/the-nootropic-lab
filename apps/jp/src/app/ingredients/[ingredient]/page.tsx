...
import { ingredients, productsJP, regionalIngredientNote, regionalTitleQualifier } from '@nootropic/data';
...
export async function generateMetadata({
  ...
  if (!ing) return {};
  const localCount = productsJP.filter((p) => ing.productsContaining.includes(p.slug)).length;
  const q = regionalTitleQualifier('jp', localCount, regionalIngredientNote('jp', ingredient));
  return {
    ...
    alternates: buildAlternates({ regionCode: 'jp', path: `/ingredients/${ingredient}/` }),
    openGraph: buildOpenGraph({ regionCode: 'jp', path: `/ingredients/${ingredient}/`, title: `${ing.name}${q} — Nootropic Guide, Dosing, Effects & FAQs`, description: `${ing.name}${q}: mechanism of action, clinical dose (${ing.clinicalDose}), human effect matrix, how to take, stacking guide, and consumer FAQs — products with international shipping.` }),
...
export default async function IngredientPage({
  ...
  const containingProducts = productsJP.filter((p) =>
  ...
  return (
    <IngredientDetail
      ...
      uiStrings={uiStrings}
      regional={{ ...regionalProps(containingProducts), note: regionalIngredientNote('jp', ing.slug) }}
    />
  );
}
