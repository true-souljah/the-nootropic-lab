import { ImageResponse } from 'next/og';
import { readFile } from 'node:fs/promises';
import { join } from 'node:path';

// Bundled Noto Sans JP subsets (official Google Fonts variable TTF, instanced
// at wght 500/800 and subset to exactly the glyphs this image renders —
// fonttools 4.63). Without bundled CJK fonts, satori dynamically fetches
// Noto Sans JP from fonts.googleapis.com at build time, which fails behind
// the pipeline runner's TLS proxy. readFile (not fetch(new URL(...))) because
// Turbopack doesn't implement local-asset fetch; cwd is apps/jp — npm
// workspaces run this app's `next build` from its own directory.
const ogFont = (file: string) => readFile(join(process.cwd(), 'src', 'app', 'og-fonts', file));

// Static OG image for all pages under this region. Next.js generates this
// PNG at build time and wires <meta property="og:image"> + Twitter
// summary_large_image tags into every page's <head>. Per-page metadata in
// pages still controls og:title and og:description.

// Static export mode needs `dynamic = 'force-static'` so Next.js generates
// the PNG at build time instead of treating the route as on-demand.
// Twemoji 'test tube' (twemoji@14.0.2, CC-BY 4.0), inlined as a data URI —
// satori otherwise fetches emoji SVGs from cdn.jsdelivr.net at build time,
// which fails behind the pipeline runner's TLS proxy.
const FLASK_ICON = 'data:image/svg+xml;base64,';

export const dynamic = 'force-static';

export const alt = 'The Nootropic Lab — 日本版';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

export default async function OpenGraphImage() {
  return new ImageResponse(
    (
      <div
        style={{
          width: '100%',
          height: '100%',
          background: 'linear-gradient(135deg, #0E5A2E 0%, #15803D 50%, #1FA34D 100%)',
          color: 'white',
          display: 'flex',
          flexDirection: 'column',
          justifyContent: 'space-between',
          padding: 72,
          fontFamily: 'system-ui, -apple-system, "Segoe UI", Roboto, sans-serif',
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div
            style={{
              width: 56,
              height: 56,
              borderRadius: 12,
              background: 'rgba(255,255,255,0.18)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              fontSize: 32,
            }}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={FLASK_ICON} width={32} height={32} />
          </div>
          <div style={{ fontSize: 30, fontWeight: 700, letterSpacing: -0.5 }}>The Nootropic Lab</div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div style={{ fontSize: 64, fontWeight: 800, lineHeight: 1.05, letterSpacing: -1.5, maxWidth: 980 }}>
            エビデンスベースの認知サプリレビュー
          </div>
          <div style={{ fontSize: 28, opacity: 0.85, fontWeight: 500 }}>
            日本版 · 国際配送 · 臨床用量監査
          </div>
        </div>

        <div style={{ fontSize: 20, opacity: 0.7, letterSpacing: 0.3 }}>jp.thenootropiclab.com</div>
      </div>
    ),
    {
      ...size,
      fonts: [
        // Passing fonts: disables next/og's bundled latin default, so latin
        // subsets must ship too or satori fetches 'Noto Sans' from Google.
        { name: 'Noto Sans', data: await ogFont('NotoSans-400-subset.ttf'), weight: 400, style: 'normal' },
        { name: 'Noto Sans', data: await ogFont('NotoSans-700-subset.ttf'), weight: 700, style: 'normal' },
        { name: 'Noto Sans JP', data: await ogFont('NotoSansJP-500-subset.ttf'), weight: 500, style: 'normal' },
        { name: 'Noto Sans JP', data: await ogFont('NotoSansJP-800-subset.ttf'), weight: 800, style: 'normal' },
      ],
    },
  );
}
