...
import { REGION_PROFILES, getRegionalHealthDisclaimer, buildRegionalBuying, hasRegionalBuyingContent, jpPrefectures } from '@nootropic/data';
import { buildGeoIndexLinks } from '@nootropic/ui';

export const REGION = REGION_PROFILES.jp;

export function regionalProps(products: Product[]) {
  return {
    region: REGION,
    products,
    geoLinks: buildGeoIndexLinks(jpPrefectures, '/prefectures').slice(0, 6),
    regulatoryNote: getRegionalHealthDisclaimer('jp'),
  };
}
...
export function regionalProductProps(product: Product) {
  const data = buildRegionalBuying(product, 'jp');
