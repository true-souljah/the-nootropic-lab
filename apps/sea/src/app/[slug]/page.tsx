...
export async function generateMetadata({
  ...
  if (!product) return {};
  const title = `${product.name} Review ${CURRENT_YEAR} — Southeast Asia Buyer's Guide`;
  const description = `Independent Southeast Asia review of ${product.name}. Score: ${product.score}/10. Clinical dosing audit and full affiliate disclosure.`;
  return {
    title,
    description,
      ...
      <article className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            {product.editorChoice && (
              <span className="editor-badge">Editor&apos;s Choice — Southeast Asia {CURRENT_YEAR}</span>
            )}
        ...
        <div className="text-sm text-gray-500">
          <a href="/best-nootropics" className="text-green-700 underline">
            ← Back to Best Nootropics Southeast Asia {CURRENT_YEAR}
          </a>
        </div>
      </article>
    </>
