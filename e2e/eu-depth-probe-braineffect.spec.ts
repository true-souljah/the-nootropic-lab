...
// EU depth probe on the region-only BRAINEFFECT
...
// content drift.
//
// Specifically: /braineffect-focus-review/ on AU. This route
// only exists in products-au.ts (BRAINEFFECT is ANZ-exclusive in
...
test.describe('EU depth probe — region-only /braineffect-focus-review/ across 3 WCAG axes', () => {
  ...
  test('target-size (WCAG 2.5.8 — 24×24 at 320px viewport) → 0 offenders', async ({ page }) => {
    ...
    await page.goto('/braineffect-focus-review/');
    ...
    expect(
      offenders,
      `Target-size offenders on BRAINEFFECT ProductDetail (< ${TARGET_MIN}×${TARGET_MIN}px):\n${JSON.stringify(offenders, null, 2)}`,
    ).toHaveLength(0);
  });
  ...
  test('focus-appearance (WCAG 2.4.13 — ≥ 2px outline) → 0 offenders across 12 Tab stops', async ({ page }) => {
    ...
    expect(
      offenders,
      `Focus Appearance offenders on BRAINEFFECT ProductDetail (no visible ≥ ${MIN_OUTLINE_WIDTH_PX}px ring):\n${JSON.stringify(offenders, null, 2)}`,
    ).toHaveLength(0);
  });
});
