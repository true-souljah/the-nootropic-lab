...
// GCC depth probe on the region-only Nahdi
...
// content drift.
//
// Specifically: /nahdi-brain-boost-review/ on AU. This route
// only exists in products-au.ts (Nahdi is ANZ-exclusive in
...
test.describe('GCC depth probe — region-only /nahdi-brain-boost-review/ across 3 WCAG axes', () => {
  test('axe (WCAG 2.1 A/AA serious + critical) → 0 violations', async ({ page }) => {
    const results = await runAxe(page, '/nahdi-brain-boost-review/');
  ...
  test('target-size (WCAG 2.5.8 — 24×24 at 320px viewport) → 0 offenders', async ({ page }) => {
    ...
    expect(
      offenders,
      `Target-size offenders on Nahdi ProductDetail (< ${TARGET_MIN}×${TARGET_MIN}px):\n${JSON.stringify(offenders, null, 2)}`,
    ).toHaveLength(0);
  });
  ...
  test('focus-appearance (WCAG 2.4.13 — ≥ 2px outline) → 0 offenders across 12 Tab stops', async ({ page }) => {
    ...
    expect(
      offenders,
      `Focus Appearance offenders on Nahdi ProductDetail (no visible ≥ ${MIN_OUTLINE_WIDTH_PX}px ring):\n${JSON.stringify(offenders, null, 2)}`,
    ).toHaveLength(0);
  });
});
