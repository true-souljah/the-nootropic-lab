...
// JP depth probe on the region-only FANCL
...
// content drift.
//
// Specifically: /fancl-brains-review/ on AU. This route
// only exists in products-au.ts (FANCL is ANZ-exclusive in
...
test.describe('JP depth probe — region-only /fancl-brains-review/ across 3 WCAG axes', () => {
  ...
  test('target-size (WCAG 2.5.8 — 24×24 at 320px viewport) → 0 offenders', async ({ page }) => {
    ...
    expect(
      offenders,
      `Target-size offenders on FANCL ProductDetail (< ${TARGET_MIN}×${TARGET_MIN}px):\n${JSON.stringify(offenders, null, 2)}`,
    ).toHaveLength(0);
  });
  ...
  test('focus-appearance (WCAG 2.4.13 — ≥ 2px outline) → 0 offenders across 12 Tab stops', async ({ page }) => {
    ...
    expect(
      offenders,
      `Focus Appearance offenders on FANCL ProductDetail (no visible ≥ ${MIN_OUTLINE_WIDTH_PX}px ring):\n${JSON.stringify(offenders, null, 2)}`,
    ).toHaveLength(0);
  });
});
