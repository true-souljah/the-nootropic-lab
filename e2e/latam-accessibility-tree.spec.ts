...
// LATAM accessibility-tree behavioral correctness probe. PR-Q67.
//
...
test.describe('LATAM accessibility-tree behavioral correctness on /best-nootropics-for-focus/', () => {
  test('skip-link is selectable by role=link with accessible name "Skip to main content"', async ({ page }) => {
    ...
    const skipLink = page.getByRole('link', { name: 'Saltar al contenido principal', exact: true });
