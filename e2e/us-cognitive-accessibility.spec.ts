...
// US cognitive-accessibility probe — WCAG 3.1.4 (Abbreviations).
// PR-Q70.
//
...
// /are-nootropics-fda-approved/ — names FDA (Food and Drug
...
const ABBREVIATIONS = [
  { acronym: 'FDA', expansion: 'Food and Drug Administration' },
  { acronym: 'DSHEA', expansion: 'Dietary Supplement Health and Education Act' },
];
...
test.describe('US — WCAG 3.1.4 Abbreviations on /are-nootropics-fda-approved/', () => {
  for (const { acronym, expansion } of ABBREVIATIONS) {
    test(`${acronym} is expanded somewhere on the page (either via <abbr title> or inline parenthetical)`, async ({
      ...
      await page.goto('/are-nootropics-fda-approved/');
